...
def load_command(args):
    ...
    print(f"\nDate range: {data['DateTime'].min().date()} to {data['DateTime'].max().date()}")
...
def visualize_command(args):
    ...
    if args.all or args.dashboard:
        ...
        if args.timeline:
            ...
            visualizer.plot_timeline(save_path=str(output_dir / 'timeline.png'))
        
        if args.net:
            print("Creating net changes plot...")
            visualizer.plot_net_changes(save_path=str(output_dir / 'net_changes.png'))
        
        if args.yearly:
            print("Creating yearly summary...")
            visualizer.plot_yearly_summary(save_path=str(output_dir / 'yearly_summary.png'))
        
        if args.heatmap:
            print("Creating activity heatmap...")
            visualizer.plot_activity_heatmap(save_path=str(output_dir / 'activity_heatmap.png'))
...
Examples:
  ...
  %(prog)s analyze --top 10 --sprints
    ...
    load_parser.add_argument('-f', '--file', default='Code frequency.csv',
                            help='CSV file to load (default: Code frequency.csv)')
    load_parser.add_argument('-s', '--summary', action='store_true',
                            help='Show summary statistics')
    load_parser.add_argument('--head', type=int, metavar='N',
                            help='Show first N rows')
    load_parser.set_defaults(func=load_command)
    
    # Analyze command
    analyze_parser = subparsers.add_parser('analyze', help='Analyze code frequency patterns')
    analyze_parser.add_argument('-f', '--file', default='Code frequency.csv',
                               help='CSV file to analyze')
    analyze_parser.add_argument('-a', '--all', action='store_true',
                               help='Show all analysis')
    analyze_parser.add_argument('--activity', action='store_true',
                               help='Show activity ratio')
    analyze_parser.add_argument('--churn', action='store_true',
                               help='Show churn statistics')
    analyze_parser.add_argument('-t', '--top', type=int, metavar='N',
                               help='Show top N active weeks')
    analyze_parser.add_argument('-y', '--yearly', action='store_true',
                               help='Show yearly statistics')
    analyze_parser.add_argument('-s', '--sprints', action='store_true',
                               help='Detect coding sprints')
    analyze_parser.set_defaults(func=analyze_command)
    
    # Visualize command
    viz_parser = subparsers.add_parser('visualize', help='Create visualizations')
    viz_parser.add_argument('-f', '--file', default='Code frequency.csv',
                           help='CSV file to visualize')
    viz_parser.add_argument('-o', '--output', default='output/visualizations',
                           help='Output directory (default: output/visualizations)')
    viz_parser.add_argument('-a', '--all', action='store_true',
                           help='Create all visualizations')
    viz_parser.add_argument('-d', '--dashboard', action='store_true',
                           help='Create complete dashboard')
    viz_parser.add_argument('--timeline', action='store_true',
                           help='Create timeline plot')
    viz_parser.add_argument('--net', action='store_true',
                           help='Create net changes plot')
    viz_parser.add_argument('--yearly', action='store_true',
                           help='Create yearly summary')
