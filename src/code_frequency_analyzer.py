...
class CodeFrequencyAnalyzer:
    ...
    def get_activity_periods(self, min_changes: int = 10) -> pd.DataFrame:
        ...
        return active[
            ["DateTime", "Additions", "Deletions", "NetChanges", "AbsChanges"]
        ]
    ...
    def detect_sprints(
        self, window_weeks: int = 4, threshold_multiplier: float = 2.0
    ) -> List[Dict]:
        ...
        df["RollingAvg"] = (
            df["AbsChanges"].rolling(window=window_weeks, center=True).mean()
        )
